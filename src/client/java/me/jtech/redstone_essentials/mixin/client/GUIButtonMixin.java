...
public abstract class GUIButtonMixin extends ClickableWidget { // Make the class abstract to avoid having to implement methods
    ...
    @ModifyArg(method = "renderWidget", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/DrawContext;drawGuiTexture(Lcom/mojang/blaze3d/pipeline/RenderPipeline;Lnet/minecraft/util/Identifier;IIIII)V"))
    public Identifier renderInjected(Identifier texture) // Replace the drawGuiTexture texture with a custom one based on the outcome of this code
    {
        if (Redstone_Essentials.shouldApplyButtonStyle) { // If the custom button style should be applied
            if (this.isHovered()) { // If the button is hovered
                return Identifier.of(Redstone_Essentials.MOD_ID, "button_highlighted"); // Provide the custom highlighted texture
            }
            return Identifier.of(Redstone_Essentials.MOD_ID, "button"); // Else, provide the custom regular texture
        }
